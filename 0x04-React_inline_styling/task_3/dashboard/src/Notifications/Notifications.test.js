...
describe('checks notification rendition', () => {
  ...
  it("renders correct list items", () => {
    ...
    expect(wrapper.find("ul").childAt(2).html()).toEqual(`<li data-notification-type=\"urgent\" class=\"urgent_137u7ef\">${getLatestNotification()}</li>`);
  });
  ...
  it('checks that menuItems displayed when displayItem is false', () => {
    ...
    expect(wrapper.contains(<p>Your Notifications</p>)).toBe(true);
  });
  ...
  it('checks that menuItem displayed when diplayItem is true', () => {
    const wrapper = shallow(<Notifications displayDrawer={true} listNotifications={listNotifications} />);
    expect(wrapper.find('.menuItem_1mp9r3t').exists()).toBe(true);
  });
