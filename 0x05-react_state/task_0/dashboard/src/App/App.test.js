...
describe('checks component state', () => {
  it('tests the default state of the component', () => {
    const wrapper = shallow(<App />);
    expect(wrapper.state().displayDrawer).toEqual(false);

    // create an instance of the wrapper so we can call the handleDisplay method
    wrapper.instance().handleDisplayDrawer();

    expect(wrapper.state().displayDrawer).toEqual(true);
  });

  it('checks if displayDrawer changes state after calling handleHideDrawer', () => {
    const wrapper = shallow(<App />);
    expect(wrapper.state().displayDrawer).toEqual(false);

    // create an instance and call handleDisplay method to change the state
    wrapper.instance().handleDisplayDrawer();
    expect(wrapper.state('displayDrawer')).toEqual(true);

    // now call the hideDisplay method to revert the state back to false
